...
-- Update close_position to set a flag before updating orders
...
-- Update the trigger to allow close_position function calls
...
AS $$
BEGIN
  -- Allow backend automation (service role)
  IF auth.role() = 'service_role' THEN
    RETURN NEW;
  END IF;

  -- Allow close_position function calls (sets app.closing_position flag)
  IF current_setting('app.closing_position', true) = 'true' THEN
    RETURN NEW;
  END IF;
