...
CREATE OR REPLACE FUNCTION public.close_position(p_order_id text, p_close_price numeric, p_net_pnl numeric, p_swap numeric, p_close_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Atomically close the order (only if still open)
  ...
  WHERE id = v_order_uuid AND status = 'open'
  ...
  -- Calculate remaining open positions PnL and margin
  FOR v_ro IN 
    SELECT o.*, s.current_price AS live_price
    FROM public.orders o
    LEFT JOIN public.symbols s ON s.id = o.symbol_id
    WHERE o.user_id = v_order.user_id AND o.status = 'open' AND o.id != v_order_uuid
  LOOP
    DECLARE
    ...
    BEGIN
      ...
      v_diff := CASE WHEN v_ro.type = 'buy' 
        THEN COALESCE(v_ro.live_price, v_ro.current_price) - v_ro.entry_price
