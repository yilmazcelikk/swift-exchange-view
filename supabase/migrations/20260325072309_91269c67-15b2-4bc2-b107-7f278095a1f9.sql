...
BEGIN
  ...
  -- Fetch USDTRY rate for BIST stock PnL conversion
  SELECT current_price INTO v_usdtry_rate FROM public.symbols WHERE name = 'USDTRY' LIMIT 1;
  IF v_usdtry_rate IS NULL OR v_usdtry_rate <= 0 THEN v_usdtry_rate := 38; END IF;
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Apply TRY/USD conversion for BIST stocks
      IF v_ro.symbol_exchange = 'BIST' THEN v_currency_divisor := v_usdtry_rate; END IF;
      v_remaining_pnl := v_remaining_pnl + (v_diff * v_ro.lots * v_cs / v_currency_divisor);
    END;
  END LOOP;
