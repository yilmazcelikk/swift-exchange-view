-- Atomic referral increment function
CREATE OR REPLACE FUNCTION public.increment_referral_usage(p_code_id uuid)
RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path = public
AS $$ UPDATE referral_codes SET usage_count = usage_count + 1 WHERE id = p_code_id; $$;

-- Fix close_position with dynamic leverage
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      v_cs := CASE
        WHEN v_ro.symbol_name = 'XAUUSD' THEN 100 WHEN v_ro.symbol_name = 'XAGUSD' THEN 5000
        WHEN v_ro.symbol_name IN ('XPTUSD','XPDUSD') THEN 100
        WHEN v_ro.symbol_name IN ('USOIL','UKOIL') THEN 1000 WHEN v_ro.symbol_name = 'NATGAS' THEN 10000
        WHEN v_ro.symbol_name IN ('BTCUSD','ETHUSD','BNBUSD','SOLUSD','XRPUSD','DOGEUSD','ADAUSD','LTCUSD','DOTUSD','AVAXUSD','LINKUSD','BCHUSD','MATICUSD','ATOMUSD','TRXUSD','SHIBUSD','UNIUSD','TONUSD','SUIUSD') THEN 1
        ...
        WHEN v_ro.symbol_name = 'COTTON' THEN 50000 WHEN v_ro.symbol_name = 'SUGAR' THEN 112000
        WHEN v_ro.symbol_name = 'COFFEE' THEN 37500 WHEN v_ro.symbol_name = 'COCOA' THEN 10
...
-- Restrict orders update: users can only update open orders they own
