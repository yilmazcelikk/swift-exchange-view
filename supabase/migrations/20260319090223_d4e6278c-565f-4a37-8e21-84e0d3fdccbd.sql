...
DECLARE
  ...
  -- Forex pairs set (only these get 100000)
  ...
  LOOP
    ...
    BEGIN
      v_cs := CASE
        ...
        WHEN v_ro.symbol_name = 'LUMBER' THEN 110
        WHEN v_ro.symbol_name = 'OATS' THEN 50
        WHEN v_ro.symbol_name = 'ORANGE' THEN 15000
        WHEN v_ro.symbol_name = 'RICE' THEN 2000
        WHEN v_ro.symbol_name = 'RUBBER' THEN 1000
        WHEN v_ro.symbol_name = ANY(v_forex_pairs) THEN 100000
        ELSE 1  -- crypto, indices, stocks all use 1
      END;
  ...
  LOOP
    ...
    IF v_net_lots > 0 AND v_net_price > 0 THEN
      v_cs := CASE
        ...
        WHEN v_sym = 'LUMBER' THEN 110
        WHEN v_sym = 'OATS' THEN 50
        WHEN v_sym = 'ORANGE' THEN 15000
        WHEN v_sym = 'RICE' THEN 2000
        WHEN v_sym = 'RUBBER' THEN 1000
